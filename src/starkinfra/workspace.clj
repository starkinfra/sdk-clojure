(ns starkinfra.workspace
  ...
  ## Parameters (required):
    - `:username` [string]: Simplified name to define the workspace URL. This name must be unique across all Stark Bank Workspaces. Ex: \"starkinfraworkspace\"
  ...
  (:import [com.starkinfra Workspace])
  (:use [starkinfra.user]
        [clojure.walk]))
...
(defn create
  ...
  ## Parameters (optional):
    ...
    - `:user` [Organization, default nil]: Organization map returned from starkinfra.user/organization. Only necessary if starkinfra.settings/user has not been set.
  ...
  ([workspace-params, user]
    (def java-workspace-params (clojure-to-java workspace-params))
    (def created-java-workspace (Workspace/create java-workspace-params (#'starkinfra.user/get-java-user user)))
    (java-to-clojure created-java-workspace)))
...
(defn query
  ...
  ## Options:
    ...
    - `:username` [string, default nil]: query by the simplified name that defines the workspace URL. This name is always unique across all Stark Bank Workspaces. Ex: \"starkinfraworkspace\"
  ...
  ([params, user] 
    (def java-params (clojure-query-to-java params))
    (map java-to-clojure (Workspace/query java-params (#'starkinfra.user/get-java-user user)))))
...
(defn page
  ...
  ([params, user] 
    (def java-params (clojure-page-to-java params))
    (def workspace-page (Workspace/page java-params (#'starkinfra.user/get-java-user user)))
    {:workspaces (map java-to-clojure (.workspaces workspace-page)), :cursor (.cursor workspace-page)}))
...
(defn update
  ...
  ## Parameters (optional):
    - `:username` [string]: Simplified name to define the workspace URL. This name must be unique across all Stark Bank Workspaces. Ex: \"starkinfra-workspace\"
    - `:name` [string]: Full name that identifies the Workspace. This name will appear when people access the Workspace on our platform, for example. Ex: \"Stark Bank Workspace\"
    - `:allowed-tax-ids` [list of strings, default nil]: list of tax IDs that will be allowed to send Deposits to this Workspace. If empty, all are allowed. ex: [\"012.345.678-90\", \"20.018.183/0001-80\"]
    - `:user` [Project or Organization, default nil]: Project or Organization map returned from starkinfra.user/project or starkinfra.user/organization. Only necessary if starkinfra.settings/user has not been set.
