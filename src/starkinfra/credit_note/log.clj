(ns starkinfra.credit-note.log
  ...
  ## Attributes (return-only):
    ...
    - `:note` [map]: CreditNote entity to which the log refers to.
...
(defn get
  "Receive a single CreditNote.Log map previously created by the Stark Infra API by its id.
  ...
  ## Return:
    - CreditNote.Log map with updated attributes"
  ([id]
...
(defn query
  "Receive a stream of CreditNote.Log maps previously created in the Stark Infra API.
  ...
  ## Options:
    ...
    - `:note-ids` [list of strings, default nil]: list of CreditNote ids to filter logs. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `user` [map, default nil]: Project or Organization map returned from starkinfra.user/project or starkinfra.user/organization. Only necessary if starkinfra.settings/user has not been set.

  ## Return:
    - stream of CreditNote.Log maps with updated attributes"
  ([]
...
(defn page
  "Receive a list of up to 100 CreditNote.Log maps previously created in the Stark Infra API and the cursor to the next page.
  ...
  ## Return:
    - map with `:content` and `:cursor`:
      - `:content`: list of CreditNote.Log maps with updated attributes
