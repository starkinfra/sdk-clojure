(ns starkinfra.pix-claim.log
  ...
  ## Attributes (return-only):
    ...
    - `:claim` [map]: PixClaim entity to which the log refers to.
...
(defn get
  "Receive a single PixClaim.Log map previously created by the Stark Infra API by its id.
  ...
  ## Return:
    - PixClaim.Log map with updated attributes"
  ([id]
...
(defn query
  "Receive a stream of PixClaim.Log maps previously created in the Stark Infra API.
  ...
  ## Options:
    ...
    - `:types` [list of strings, default nil]: filter retrieved maps by types. ex: [\"created\", \"failed\", \"delivering\", \"delivered\", \"confirming\", \"confirmed\", \"success\", \"canceling\", \"canceled\"]
    - `:claim-ids` [list of strings, default nil]: list of PixClaim ids to filter retrieved maps. ex: [\"5656565656565656\", \"4545454545454545\"]
    - `:ids` [list of strings, default nil]: Log ids to filter PixClaim Logs. ex: [\"5656565656565656\"]
    - `user` [map, default nil]: Project or Organization map returned from starkinfra.user/project or starkinfra.user/organization. Only necessary if starkinfra.settings/user has not been set.

  ## Return:
    - stream of PixClaim.Log maps with updated attributes"
  ([]
...
(defn page
  "Receive a list of up to 100 PixClaim.Log maps previously created in the Stark Infra API and the cursor to the next page.
  ...
  ## Return:
    - map with `:content` and `:cursor`:
      - `:content`: list of PixClaim.Log maps with updated attributes
